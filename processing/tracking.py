...
class MetricaPassingNetwork(PassingNetworkBuilder, ABC):
    ...
    def compute_total_minutes(self):
        # Look for the minute of either the first substitution or sent-out player, otherwise use full match.
        # As Metrica does not provide an event type for substitutions, we use tracking data to know when the first player is introduced in the pitch (player number 12).
