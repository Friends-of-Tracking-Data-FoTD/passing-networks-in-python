...
def read_event_data(DATADIR, game_id):
    ...
    '''
    eventfile = '/Sample_Game_{0}/Sample_Game_{1}_RawEventsData.csv'.format(game_id, game_id)
...
def tracking_data(DATADIR, game_id, teamname):
    ...
    '''
    teamfile = '/Sample_Game_{0}/Sample_Game_{1}_RawTrackingData_{2}_Team.csv'.format(game_id, game_id, teamname)

    # First:  deal with file headers so that we can get the player names correct
    csvfile =  open('{0}/{1}'.format(DATADIR, teamfile), 'r') # create a csv file reader
